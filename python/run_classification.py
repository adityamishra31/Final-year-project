...
classifiers = [('Decision tree', DT()), ('Random forest', RF(n_estimators=100))]  # Add more classifiers if needed

write_results = True
name_result_file = 'results_java.csv'

if write_results:
    write_header = not os.path.exists(name_result_file)
    results = open(name_result_file, 'a')
...
for dataset in datasets:
    ...
    for technique in techniques:
        ...
        if technique in ['iBCM', 'BIDE', 'SPADE', 'PrefixSPAN']:
            ...
            classify(technique, classifiers, [])
